...
|`parameter`
| Parameter file, e.g., a parameter set in SSV format.
|`system`
...
This appendix describes the format of the EXP experiment files that can be included in an FMU as part of the layered standard.
The EXP format is a simple XML-based format that allows the description of experiment setups, including parameters, stimuli, and references.

The EXP format is a valid XML file that contains a root element `<Experiments>` with the following attributes:
