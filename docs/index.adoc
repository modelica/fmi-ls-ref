...
NOTE: This appendix is non-normative, and is provided for informational purposes only.
It is expected that this appendix will be moved to a separate document in the future.
...
Each experiment setup is described by an `<Experiment>` element in the experiments file.
...
The following tables describe the attributes of each experiment element in more detail.
